...
function scheduleReview(card, grade, reviewTime = new Date()) {
  ...
  if (grade === GRADES.AGAIN) {
    ...
    if (card.repetition === 0) {
      ...
      const multiplier =
        grade === GRADES.HARD ? 1.2 : grade === GRADES.GOOD ? next.ease : next.ease * 1.3;
...
- **Charts**:
  - _Mastery heatmap_ (Chart.js matrix plugin or custom canvas): X-axis units/topics, Y-axis conjugation tense/case; cell color from accuracy average. Data from `srs_items.tags` + `srs_history`.
  - _Review load forecast_ (stacked bar): Query `srs_items` by `nextReview` → group by due date next 14 days.
  - _Accuracy over time_ (line): Rolling 7-day average accuracy from `srs_history`.
  - _Time spent_ (donut): Aggregate `responseTime` per game context.
